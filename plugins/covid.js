...
if (Config.WORKTYPE == 'private') {
  Amdi.addrex({pattern: 'covid ?(.*)', fromMe: true, desc: LANG.COVID_DESC,  deleteCommand: false}, async (message, match) => {
  ...
  Amdi.addrex({pattern: 'covid ?(.*)', fromMe: false, desc: LANG.COVID_DESC}, async (message, match) => {
    if (match[1] === '') return await message.reply(LANG.NEED_CON);
      const url = `https://coronavirus-19-api.herokuapp.com/countries/${match[1]}`;
            const response = await got(url);
            const jsun = JSON.parse(response.body);
